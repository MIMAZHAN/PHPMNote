...
session_start();
if(isset($_POST["title"]) && isset($_POST["text"]) && isset($_POST["dateTime"]) && !empty($_POST["title"]) && !empty($_POST["text"]) && !empty($_POST["dateTime"]))
{	
	if(isset($_SESSION["logged"]) && isset($_SESSION["userId"]))
	{
		if($_SESSION["logged"] == true)
		{
			...
			do{
				...
				$selectQuery = "SELECT ID FROM $dbNotesTable WHERE PUBLICID ='" . $publicId . "' LIMIT 1";
			...
			$querySetPublicId = "UPDATE $dbNotesTable SET PUBLICID='" . $publicId . "' WHERE USER=$userId AND TRIM(TITLE) ='" . $title. "' AND TRIM(DATETIME)='" . $dateTime . "'";
